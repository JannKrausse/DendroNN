...
def create_neuromorse_dataset():
    ...
    for idx,word in enumerate(Top50List):
        ...
        for i,ch in enumerate(word):
            ...
            Spikes['t'] += time 
            time = Spikes[-1][0]+(1+letter_space)  ################################## ACCORDING TO MY ANALYSIS THIS DIFFERS FROM THE CONSTRUCTION OF THE TEST DATA!!!################################## ACCORDING TO MY ANALYSIS THIS DIFFERS FROM THE CONSTRUCTION OF THE TEST DATA!!!
    ...
    for word in cleaned_test_subset:
        ...
        for ch in word:
            if ch in Morse_Spike_Dict:
                ...
                time = char_spikes['t'][-1] + (1 + letter_space)  # MY CORRECTION, SEE REASON ABOVE
            else:
    ...
    # Load validation.txt
    with open('neuromorse_data/NeuroMorse/data/Validation/validation.txt', 'r', encoding='utf8') as f:
        validation_subset = f.read().lower().split()

    # Remove punctuation from each word
    cleaned_validation_subset = [word.translate(translator) for word in validation_subset]
    ...
    for i in range(len(ValidationSpikeDataset)):
        ...
        ValidationSpikeDataset[i] = F.pad(ValidationSpikeDataset[i], (0, 0, 0, padding), mode='constant', value=0)  # Pad at the end
