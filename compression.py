...
def test_model_with_compression(
    model,
    test_loader,
    train_loader,
    val_loader,
    ...
    original_trainer = Trainer(
        ...
        limit_test_batches=1 if smoke_test else 1.0,
    )
    ...
    print(
        f"  Target pruning: {pruning_level*100:.1f}%, Actual pruning: {compression_stats['sparsity_percentage']:.2f}%"
    )
    print(
    ...
    print(
        f"Compressed model test accuracy: {results['compressed'].get('test_acc', 'N/A')}"
    )
...
def analyze_compression(model, layer_name="out"):
    ...
    print(
        f"  Non-zero weights: {nonzero_weights:,} ({results['density_percentage']:.2f}%)"
    )
...
def quantize_weights_manual(weights, bit_width):
    ...
    if (
        torch.abs(min_val) < 1e-8 and torch.abs(max_val) < 1e-8
    ...
    print(
        f"Quantization to {bit_width} bits ({method}): {len(unique_vals)} unique values (target ≤{n_levels})"
    )
    print(
        f"L2 quantization error norm: {((final_quantized - weights)**2).sum().item()**0.5:.6f}"
    )
...
class IterativePruningCallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if (
            ...
            and (current_epoch - self.start_epoch) % adaptive_interval == 0
            and self.current_pruning_total < self.target_pruning_level
        ):
            ...
            if self.current_pruning_total < 1.0:
                amount_to_prune = (
                    target_pruning_total - self.current_pruning_total
                ) / (1.0 - self.current_pruning_total)
            else:
                amount_to_prune = 0.0
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if hasattr(layer, "weight_mask"):
            ...
            print(
                f"Final pruning: {final_pruning*100:.2f}% ({zero_count:,}/{total_count:,} weights)"
            )
...
def compress_model(
    ...
    """
    print(
        f"Starting compression: {pruning_level*100:.1f}% iterative pruning, {bit_width}-bit quantization"
    )
