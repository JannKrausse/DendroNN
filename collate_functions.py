...
class CollateFN:
    def __init__(self, slice_amounts=False, num_slices=7, slicing_thrs=None, slicing_thrs_maxs=None, include_zero_slice=False, jitter_std=0.0, dropout_prob=0.0, poisson_lambda=0.0, padding_len=400, permute_data=False, permutation_seed=None):
        ...
        if poisson_lambda == 'none':  # in NeuorMorse: low=0.05, high=0.1 --> translates to low=7.5, high=14.2 (different implementations)
        ...
        if self.permute_data:
            ...
            self.permutation_idx = torch.randperm(padding_len, generator=self.permutation_generator)
    ...
    def __call__(self, batch):
        ...
        if isinstance(targets[0], int):  # sMNIST
            ...
            input_flat = input_data.flatten(start_dim=2)  # same as torch.Tensor.view(-1, 1) as often done for sMNIST
            input_data = input_flat.permute(0, 2, 1)  # permute to (batch_size, seq_len, in_shape), later reshaped to (seq_len, batch_size, in_shape)
            ...
            try:
                ...
                input_data = tuple(np.pad(arr, pad_width=[(0, self.padding_len - arr.shape[0])] + [(0, 0)] * (arr.ndim - 1), mode='constant') for arr in input_data)
        ...
        if self.slice_amounts:
            if input_data.shape[0] < 128:
                warnings.warn("Batch size is less than 128, data_max might not be representative. Consider increasing the batch size.")
            if self.data_max is None:
                self.data_max = input_data.max()
                if self.slicing_thrs is None:
                    self.slicing_thrs = np.linspace(0, self.data_max / 4, self.num_slices)
                else:
            ...
            if not self.include_zero_slice:
                sliced_data = torch.zeros((input_data.shape[0], input_data.shape[1], input_data.shape[2] * self.num_slices))
            else:
                sliced_data = torch.zeros((input_data.shape[0], input_data.shape[1], input_data.shape[2] * (self.num_slices + 1)))
                sliced_data[:, :, input_data.shape[2]*self.num_slices:input_data.shape[2]*(self.num_slices+1)] = input_data == 0
            for i in range(self.num_slices):
                sliced_data[:, :, input_data.shape[2]*i:input_data.shape[2]*(i+1)] = (input_data > self.slicing_thrs[i]) * (input_data < self.slicing_thrs_maxs[i]) #* (input_data > 0)
        ...
        input_data = input_data.permute(1, 0, 2).float()  # shape: (seq_len, batch_size, ...)
    ...
    def apply_noise(self, data):
        ...
        if self.jitter_std > 0:
            jitter_amount = torch.normal(mean=0.0, std=self.jitter_std, size=(len(ones),))
            jitter_amount = jitter_amount.round().long()
            new_ones[:, self.jitter_axis] += jitter_amount
            # Clamp to valid range
            new_ones[:, self.jitter_axis] = new_ones[:, self.jitter_axis].clamp(0, data.shape[self.jitter_axis] - 1)
        ...
        if self.poisson_lambda > 0:
            for i in range(data.shape[0]):  # for every sample
                num_noise = torch.poisson(torch.tensor([self.poisson_lambda])).long().item()
                if num_noise > 0:
...
class OnlySomeClassesCollateFn(CollateFN):
    def __init__(self, class_ids=None, rename_labels=True, redraw_each_batch=False, num_classes=None, num_kept_classes=None, slice_amounts=False, num_slices=7, slicing_thrs=None, slicing_thrs_maxs=None, include_zero_slice=False, surrogate_labels=None, jitter_std=0.0, dropout_prob=0.0, poisson_lambda=0.0, padding_len=400, permute_data=False, permutation_seed=None):
        super().__init__(slice_amounts=slice_amounts, num_slices=num_slices, slicing_thrs=slicing_thrs, slicing_thrs_maxs=slicing_thrs_maxs, include_zero_slice=include_zero_slice, jitter_std=jitter_std, dropout_prob=dropout_prob, poisson_lambda=poisson_lambda, padding_len=padding_len, permute_data=permute_data, permutation_seed=permutation_seed)
        ...
        if (self.class_ids is None and not self.redraw_each_batch) or (self.class_ids is not None and self.redraw_each_batch):
            raise ValueError("Either class_ids are pre-defined or randomly sampled.")
        if self.redraw_each_batch and (self.num_classes is None or self.num_kept_classes is None):
            raise ValueError("If labels are randomly sampled then num_classes and num_kept_classes should be defined.")
    ...
    def additional_stuff(self, data, targets):
        ...
        if self.redraw_each_batch:
            ...
            self.class_ids = random.sample(range(self.num_classes), self.num_kept_classes)
