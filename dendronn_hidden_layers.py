...
class DendroNNHiddenLayer(nn.Module):
    def __init__(self, input_size, output_size, spines_per_output_channel, reshape_output=True):
        super().__init__()
        ...
        synapse_mask = torch.zeros((self.output_size * self.spines_per_output_channel,
                                self.input_size))
        ...
        self.weights = nn.Parameter(torch.ones(self.output_size * self.spines_per_output_channel,
                                                self.input_size),
                                    requires_grad=False)

    def rewire_synapses(self, rewiring_mask):
        repeated_rewiring_mask = rewiring_mask.repeat_interleave(self.spines_per_output_channel)
        for i in range(self.output_size * self.spines_per_output_channel):
    ...
    def __call__(self, _input):
        ...
        if self.reshape_output:
            output = torch.reshape(output, (*_input.shape[:-1], self.output_size, self.spines_per_output_channel))
