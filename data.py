...
class CustomAudioUniformNoiseTransform:
    def __init__(self, sensor_size, nb_noise_events, random=False):
        ...
        self.structured_type = np.dtype([('t', np.int32), ('x', np.int32), ('p', np.int32)])
    ...
    def __call__(self, e):
        ...
        random_channel = np.random.randint(0, self.sensor_size[0], (self.nb_noise_events,))
        for i in range(self.nb_noise_events):
            noise_events = np.append(noise_events, np.array([(random_time[i], random_channel[i], 1)],
                                                            dtype=self.structured_type))
...
class CustomAudioDenoiseTransform:
    ...
    def tonic_call(self, events):
        ...
        for event in events:
            ...
            if (
                (x > 0 and timestamp_memory[x - 1] > t)
                or (x < width - 1 and timestamp_memory[x + 1] > t)
            ):
...
class OtherAudioUniformNoiseTransform(tonic.transforms.UniformNoise):
    ...
    def __call__(self, events):
        ...
        e['p'] = np.ones(len(e))  # UniformNoise transform does not work with sensor_size=(700, 1, 1) because noise events have polarity 0 instead of 1
        return e
...
class CustomAudioSpatialJitterTransform:
    ...
    def __call__(self, e):
        ...
        spatial_jitter = np.random.normal(loc=0, scale=self.var_x, size=(len(e),)).astype(np.int32)
...
class TonicDataset:
    def __init__(
        ...
        self.save_path = '/snailta/Datasets/'
        ...
        self.sensor_size = (max(int((sensor_size[0] - self.num_crop_pixel) * spat_ds_fac), 1), max(int((sensor_size[1] - self.has_one_spatial_dim * self.num_crop_pixel) * spat_ds_fac), 1), sensor_size[2])

        self.transforms = []

        self.aug_og_data = aug_og_data
        self.aug_kwargs = aug_kwargs
        if self.aug_og_data:  # transforms and parameter values taken from event-by-event ssm repo
            if self.dataset_name == "SHD":
                if self.aug_kwargs is None:
                    ...
                    self.transforms.extend(
                        [
                            tonic.transforms.DropEvent(p=augmentation_kwargs["drop_event_prob"]),
                            DropEventChunk(p=0.3,
                                        max_drop_size=augmentation_kwargs["max_drop_chunk"]),
                            Jitter1D(sensor_size=self.not_downsampled_sensor_size, var=augmentation_kwargs["var_x"]),
                            tonic.transforms.TimeSkew(coefficient=(1 / augmentation_kwargs["time_skew"],
                                                                augmentation_kwargs["time_skew"]),
                                                    offset=0),
                            tonic.transforms.TimeJitter(std=augmentation_kwargs["std_t"],
                                                        clip_negative=False,
                                                        sort_timestamps=True),
                            OtherAudioUniformNoiseTransform(sensor_size=self.not_downsampled_sensor_size,
                    ...
                    if "max_drop_chunk" in augmentation_kwargs.keys():
                        self.transforms.extend([
                            DropEventChunk(p=0.3, max_drop_size=augmentation_kwargs["max_drop_chunk"]),
                        ])
                    if "time_skew" in augmentation_kwargs.keys():
                        self.transforms.extend([
                            tonic.transforms.TimeSkew(coefficient=(1 / augmentation_kwargs["time_skew"],
                                                        augmentation_kwargs["time_skew"]), offset=0),
                        ])
            else:
        ...
        if self.num_crop_pixel > 0:
            self.transforms.extend(
            [
                Custom1DCropTransform(sensor_size=self.not_downsampled_sensor_size, num_crop_pixel=num_crop_pixel),
            ]
        )
        self.transforms.extend(
            [
                tonic.transforms.Downsample(spatial_factor=spat_ds_fac),
                tonic.transforms.ToFrame(sensor_size=self.sensor_size, time_window=self.time_window) #n_time_bins=nb_bins),
            ]
        )
        ...
        if self.denoise:
            filter_time = 5000
            if self.dataset_name == 'SHD':
                denoise_transform = CustomAudioDenoiseTransform(filter_time=filter_time, mode=self.denoise_mode)
            else:
                raise Exception(f'For dataset {self.dataset_name} denoising is not implemented yet.')
            
        if self.denoise_mode == 'tonic':
            self.transforms = self.transforms[:-2] + [denoise_transform] + self.transforms[-2:]
        ...
        if dataset_name == 'SHD':
            if self.val_on_test:
                self.data_dict['train_set'] = self.dataset(self.save_path, transform=transform, train=True)
                self.data_dict['valid_set'] = self.dataset(self.save_path, transform=transform, train=False)
            else:
                og_train_set = self.dataset(self.save_path, transform=transform, train=True)
                ...
                self.data_dict['test_set'] = self.dataset(self.save_path, transform=transform, train=False)
        else:
    ...
    def apply_augmentation(self, task_type, augmentation_kwargs):
        ...
        if not isinstance(task_type, list):
            if task_type == 'audio':
                self.create_augmented_data('audio_uniform_noise', augmentation_kwargs['num_noise_events'])
                self.create_augmented_data('drop_event', augmentation_kwargs['drop_event_prob'])
                self.create_augmented_data('time_jitter', augmentation_kwargs['std_t'])
                self.create_augmented_data('audio_spatial_jitter', augmentation_kwargs['var_x'])
            elif task_type == 'visual':
                self.create_augmented_data('random_crop', augmentation_kwargs['size_share'])
                self.create_augmented_data('uniform_noise', augmentation_kwargs['num_noise_events'])
                self.create_augmented_data('drop_event', augmentation_kwargs['drop_event_prob'])
                self.create_augmented_data('time_jitter', augmentation_kwargs['std_t'])
                self.create_augmented_data('spatial_jitter', augmentation_kwargs['var_x'])
            elif task_type == '0':
                self.create_augmented_data('random_crop', augmentation_kwargs['size_share'])
            elif task_type == '1':
                self.create_augmented_data('uniform_noise', augmentation_kwargs['num_noise_events'])
    ...
    def aug_random_crop(self, size_share):
        aug_sensor_size = (int(size_share*self.sensor_size[0]), int(size_share*self.sensor_size[1]),
                           self.sensor_size[2])
        aug_transforms = [
            tonic.transforms.RandomCrop(sensor_size=self.sensor_size,
                                        target_size=(aug_sensor_size[0], aug_sensor_size[1])),
            tonic.transforms.ToFrame(sensor_size=aug_sensor_size, n_time_bins=self.nb_bins),
        ]
        return aug_transforms

    def aug_uniform_noise(self, num_noise_events):
        return [tonic.transforms.UniformNoise(sensor_size=self.sensor_size,
                                              n=num_noise_events)]

    def aug_audio_uniform_noise(self, num_noise_events):
        return [CustomAudioUniformNoiseTransform(sensor_size=self.sensor_size, nb_noise_events=num_noise_events)]
    ...
    def aug_spatial_jitter(self, var_x, var_y=None):
        if var_y is None:
            var_y = var_x
        return [tonic.transforms.SpatialJitter(sensor_size=self.sensor_size, var_x=var_x, var_y=var_y,
                                               clip_outliers=True)]

    def aug_audio_spatial_jitter(self, var_x):
        return [CustomAudioSpatialJitterTransform(sensor_size=self.sensor_size, var_x=var_x, clip_outliers=True)]
